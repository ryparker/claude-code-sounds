...
const resolveThemeSoundPath = (name, file) =>
  lib.resolveThemeSoundPath(name, file, paths);
    ...
    super(
      {
        render() {
          ...
          if (this.state === "submit") {
            ...
            lines.push(
              `${color.green(p.S_STEP_SUBMIT)}  ${myMessage} ${color.dim(`(${totalSel} assigned)`)}`,
            );
            return lines.join("\n");
          }
          ...
          const noMarginCols = Math.floor(
            (termCols - linePrefix - labelWidth) / colWidth,
          );
          ...
          if (noMarginCols >= totalHooks) {
            ...
            needsHScroll = true;
            const withMarginCols = Math.floor(
              (termCols - linePrefix - labelWidth - 2) / colWidth,
            );
            if (withMarginCols >= 1) {
          ...
          if (needsHScroll) {
            ...
            if (cursorCol >= colStart + visibleCols)
              colStart = cursorCol - visibleCols + 1;
            colStart = Math.max(
              0,
              Math.min(colStart, totalHooks - visibleCols),
            );
          } else {
            colStart = 0;
          }
          ...
          const showRightArrow =
            needsHScroll && colStart + visibleCols < totalHooks;
          const leftMargin = needsHScroll
            ? showLeftArrow
              ? color.dim("\u25C2")
              : " "
            : "";
          const rightMargin = needsHScroll
            ? showRightArrow
              ? color.dim("\u25B8")
              : " "
            : "";
          ...
          for (let c = colStart; c < colStart + visibleCols; c++) {
            ...
            headerLine +=
              c === cursorCol ? color.cyan(color.bold(abbr)) : color.dim(abbr);
          }
          ...
          const maxVisible = Math.max(
            5,
            termRows - lines.length - reservedLines - 2,
          );
          ...
          for (
            let i = scrollTop;
            i < myRows.length && visibleCount < maxVisible;
            i++
          ) {
            const row = myRows[i];
            if (row.type === "header") {
              ...
              lines.push(
                `${color.gray(p.S_BAR)}  ${color.gray(hdr + "\u2500".repeat(dashLen))}`,
              );
            } else {
              ...
              lines.push(
                `${color.gray(p.S_BAR)}  ${pointer} ${styledLabel}${cellsStr}`,
              );
            }
            visibleCount++;
          }
          ...
          lines.push(`${color.gray(p.S_BAR)}`);
          const helpFull =
            "\u2191\u2193 sounds \u00B7 \u2190\u2192 hooks \u00B7 space toggle \u00B7 p preview \u00B7 a column all \u00B7 enter done";
          const helpShort =
            "\u2191\u2193/\u2190\u2192 move \u00B7 space \u00B7 p play \u00B7 a all \u00B7 enter";
          const helpText =
            helpFull.length + 5 <= termCols ? helpFull : helpShort;
    ...
    this.on("cursor", (action) => {
      if (action === "up") {
        if (this._soundIndices.length > 0) {
          this._cursorRow =
            (this._cursorRow - 1 + this._soundIndices.length) %
            this._soundIndices.length;
        ...
        this._cursorCol =
          (this._cursorCol - 1 + this._hooks.length) % this._hooks.length;
      } else if (action === "right") {
        this._cursorCol = (this._cursorCol + 1) % this._hooks.length;
      } else if (action === "space") {
        this._grid[this._cursorRow][this._cursorCol] =
          !this._grid[this._cursorRow][this._cursorCol];
      }
    });
...
async function quickInstall(theme) {
  ...
  p.log.success(
    `Installed ${color.bold(theme.display)} — ${total} sounds across ${categories} hooks.`,
  );
}
...
function showList() {
  ...
  for (const t of themes) {
    ...
    console.log(
      `    ${t.name} — ${t.description} (${t.soundCount} sounds)${src}`,
    );
  }
  console.log();
}
  ...
  if (existing && !autoYes) {
    ...
    const muteOption = muted
      ? {
          value: "unmute",
          label: "Unmute sounds",
          hint: "Sounds are currently muted",
        }
      : {
          value: "mute",
          label: "Mute sounds",
          hint: "Silence sounds without uninstalling",
        };
    ...
    const dndOption = dnd
      ? {
          ...
          hint: "Stop auto-muting during calls",
        }
      : {
          value: "dnd-on",
          label: "Enable Do Not Disturb",
          hint: "Auto-mute during video calls",
        };
    ...
    const action = await p.select({
      ...
      options: [
        ...
        dndOption,
        {
          value: "fresh",
          label: "Fresh install",
          hint: "Start over from scratch",
        },
        {
          value: "uninstall",
          label: "Uninstall",
          hint: "Remove all sounds and hooks",
        },
      ],
    });
    ...
    if (action === "dnd-on") {
      ...
      p.outro(
        "Do Not Disturb enabled. Edit ~/.claude/sounds/.dnd to customize.",
      );
      return;
    }
    ...
    options: [
      {
        value: "quick",
        label: "Quick install",
        hint: "One theme, all defaults",
      },
      {
        value: "custom",
        label: "Custom mix",
        hint: "Pick sounds per hook from multiple themes",
      },
    ],
  });
...
if (flags.has("--help") || flags.has("-h")) {
  ...
  lib.setDnd(true, paths);
  console.log(
    "  Do Not Disturb enabled. Sounds auto-mute when video call apps are detected.",
  );
